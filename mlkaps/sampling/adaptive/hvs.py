...
Definition of the Hierarchical Variance Sampling (HVS) adaptive sampling method
...
def _error_variance(confidence: float, input: pd.DataFrame) -> float:
    """
    Compute an upper bound of the variance of the input dataframe.

    Args:
        confidence (float): The desired confidence (alpha) for the variance.
            For a confidence interval of 95%, this value should be 0.05.
        input (pd.DataFrame): The data on which to compute the variance upper bound.

    Returns:
        float: An upper bound of the input's variance.
    """
...
def _error_cov(confidence: float, input: pd.DataFrame) -> float:
    """
    Compute an upper bound of the covariance of the input dataframe.

    Args:
        confidence (float): The desired confidence (alpha) for the covariance.
            For a confidence interval of 95%, this value should be 0.05.
        input (pd.DataFrame): The data on which to compute the covariance upper bound.

    Returns:
        float: An upper bound of the input's covariance.
    """
...
def _get_error_function(name: str) -> Callable[[pd.DataFrame], float]:
    ...
    Returns:
        Callable[[pd.DataFrame], float]: The requested error function.

    Raises:
        SamplerError: If the name doesn't correspond to a known error function.
    """
...
class HVSPartition:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            bounds (dict): A dictionary containing the bounds of the partitions for each feature,
                stored as feature: [low_bound, high_bound].
            samples (pd.DataFrame): A DataFrame containing the samples in this partition.
            node_id (int): The id of the node in the DecisionTree corresponding to this leaf.
            error_function (Callable[[pd.DataFrame], pd.DataFrame]): The function to use to compute
                the error in this partition.
            objective (str): The name of the features in the samples to consider as
                the objective to compute the error.
            size_metric (Callable[[HVSPartition], float]): The method used to compute the size of
                the partition.
        """
    ...
    def __str__(self):
        """
        Return a string representation of the HVS partition.

        Returns:
            str: A formatted string showing the partition's bounds, samples, size, error, and score.
        """
...
def _std_partition_size(partition: HVSPartition) -> float:
    ...
    Args:
        partition (HVSPartition): The partition to compute the size for.

    Returns:
        float: The size of the partition.
    """
    size = 1
...
def _get_size_metric(name: str) -> Callable[[HVSPartition], float]:
    """
    Get a size metric function by name.

    Args:
        name (str): The name of the size metric.

    Returns:
        Callable[[HVSPartition], float]: The requested size metric function.

    Raises:
        SamplerError: If the name doesn't correspond to a known size metric.
    """
...
class HVSPartitionner:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            features (dict): A dictionary containing all the features to use to partition the samples.
            objective (str): The name of the features in the samples to consider as the objective.
            tree_params (dict): The parameters to pass to the DecisionTreeRegressor.
            error_metric (str | Callable[[pd.DataFrame], float], optional): The error metric to use
                to compute the error of each partition, defaults to "variance". If a string is passed,
                the partitioner will try to fetch the corresponding metric. Callables are used directly.
            size_metric (str | Callable[[HVSPartition], float], optional): The size metric to use
                to compute the size of each partition, defaults to "size". If a string is passed,
                the partitioner will try to fetch the corresponding metric. Callables are used directly.
        """
    ...
    def partition(self, samples: pd.DataFrame) -> list[HVSPartition]:
        """
        Run the partitioner on the given samples.

        Args:
            samples (pd.DataFrame): The samples to partition.

        Returns:
            list[HVSPartition]: A list of the built partitions.
        """
    ...
    def _build_tree(self, samples: pd.DataFrame):
        ...
        Args:
            samples (pd.DataFrame): The samples to fit the tree on.

        Returns:
            DecisionTreeRegressor: The fitted decision tree.
        """
    ...
    def _make_node_children(
        ...
        """
        Fetch the two children of a node in the decision tree and compute their bounds.

        Args:
            tree_features (dict): A dictionary containing the feature used at each node.
            thresholds (dict): The threshold used at each node of the tree.
            children_left (dict): The left children of each node.
            children_right (dict): The right children of each node.
            parent_node (tuple[int, dict]): The parent node id and its bounds.

        Returns:
            list: A list containing a tuple (node_id, bounds) for the two children of the parent node.
        """
    ...
    def _parse_tree(self, tree: DecisionTreeRegressor):
        ...
        Args:
            tree (DecisionTreeRegressor): The tree to parse.

        Yields:
            tuple[int, dict]: A leaf node id in the tree, and a dictionary containing the bounds for each feature.
        """
    ...
    def _build_partitions(self, tree: DecisionTreeRegressor, samples: pd.DataFrame) -> list[HVSPartition]:
        ...
        Args:
            tree (DecisionTreeRegressor): The decision tree to retrieve the partitions from.
            samples (pd.DataFrame): The samples used to train the decision tree.

        Returns:
            list[HVSPartition]: A list of all the partitions retrieved from the tree.
        """
    ...
    def _remove_extra_samples(
        ...
        """
        Remove extra samples from partitions to ensure exactly n_samples samples.

        We must have exactly n_samples samples, so we remove any extra samples from the
        partitions with the lowest variance.

        Args:
            n_samples (int): The target number of samples.
            partitions (list[HVSPartition]): List of partitions to remove samples from.
            partitions_samples (dict[int, int]): Dictionary mapping partition node IDs to number of samples.

        Returns:
            dict[int, int]: Updated dictionary with adjusted sample counts.
        """
    ...
    def distribute_samples(self, partitions: list[HVSPartition], n_samples: int) -> list[tuple[int, HVSPartition]]:
        ...
        Args:
            partitions (list[HVSPartition]): A list of the partitions to distribute the samples on.
            n_samples (int): The number of samples to distribute on the partitions.

        Returns:
            list[tuple[int, HVSPartition]]: A list of tuple(int, HVSPartition), where the first element is the number of
...
class HVSampler(AdaptiveSampler):
    """
    Hierarchical Variance Sampling (HVS) is an adaptive sampling strategy based on
    ...
    partition size and variance. If the sampled function is multi-output, a decision tree is created for each
    output, and the new samples are split between each tree.

    This class should be used in conjunction with the AdaptiveSamplingOrchestrator class for
    automated adaptive sampling. If used directly, the user must handle the stopping criteria and the final dataset itself.
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            variables (dict | None): A dictionary containing the ValueContainers of the variables to sample.
            error_metric (str): Name of the error metric to use, one of ["variance", "cov"]. Defaults to "variance".
            min_samples_per_leaf (int): The minimum number of samples per leaf in the decision tree. Defaults to 15.
        """
    ...
    def set_variables(self, variables: dict):
        """
        Set the variables for the sampler and build bounded features.

        Args:
            variables (dict): A dictionary containing the ValueContainers of the variables to sample.
        """
    ...
    def dump(self, output_directory: Path):
        """
        Output the median errors to a csv file in the given directory as "hvs_errors.csv".

        Args:
            output_directory (pathlib.Path): The path where to save the error.
        """
    ...
    def _verify_arguments(
        ...
        """
        Verify the arguments passed to the sampler.

        Args:
            n_samples (int): The number of samples to generate.
            execution_func (Callable[[pd.DataFrame], pd.DataFrame]): The execution function.
            features (dict): The features dictionary.

        Raises:
            SamplerError: If any of the parameters is invalid.
        """
    ...
    def _lhs_bootstrap(self, execution_func: Callable[[pd.DataFrame], pd.DataFrame], n_samples: int) -> pd.DataFrame:
        ...
        Args:
            execution_func (Callable[[pd.DataFrame], pd.DataFrame]): The execution function to evaluate the samples.
            n_samples (int): The number of samples to take using LHS.

        Returns:
            pd.DataFrame: A dataframe of samples.
        """
    ...
    def sample(
        ...
        """
        Sample new points from the given function, based on existing data.
        If data is None, the sampler will bootstrap the dataset via LHS sampling using n_samples.

        Args:
            n_samples (int): The number of samples to generate, will be split between the objectives.
                Must be >= 0. If == 0, this function will return the input data and no sampling will be performed.
            samples (pd.DataFrame | None): A dataframe of already sampled points, or None.
                If None, the sampler will bootstrap the dataset by using LHS sampling.
            execution_func (Callable[[pd.DataFrame], pd.DataFrame]): The callable function to sample.
        ...
        Returns:
            pd.DataFrame: A dataframe containing the extended dataset.
        """
    ...
    def _sample_for_all_objectives(
        ...
        Args:
            n_samples (int): The number of samples to generate.
            samples (pd.DataFrame): The available samples.
            execution_func (Callable[[pd.DataFrame], pd.DataFrame]): The function to sample.

        Returns:
            tuple[pd.DataFrame, pd.DataFrame]: An extended dataset, and the errors for each objective.
        """
    ...
    def partition(
        ...
        Args:
            samples (pd.DataFrame): The samples to build the partitions with.
            objective (str): The name of the feature in the samples to consider as the objective.
            n_samples (int): The number of samples to distribute over the partitions.
            split_on (list[str] | None): The features to consider when building the decision tree.
                Defaults to None. If None, takes the features defined in the constructor or using set_variables.
            min_samples_per_leaf (int): The minimum number of samples per leaf in the decision tree.
                Defaults to 15.

        Returns:
            tuple[list[HVSPartition], float]: A list of partitions, and the median error.
        """
        ...
        partitions = partitioner.partition(samples)
        self.final_partitions = partitions
        self.final_tree = partitioner.tree
        median_error = np.median([p.error for p in partitions])

        partitions = partitioner.distribute_samples(partitions, n_samples)

        return partitions, median_error

    def sample_partitions(self, decorated_partitions: list[HVSPartition]) -> pd.DataFrame:
        """
        Sample each partition using LHS, and output a dataframe with all the samples.
        The number of samples is given by the "n_samples" key of each partition.

        Args:
            decorated_partitions (list[HVSPartition]): A list of partitions to sample from.

        Returns:
            pd.DataFrame: A dataframe containing all the sampled points.
        """
        res = []
    ...
    def _maybe_map_variables(self, samples: pd.DataFrame, reverse: bool = False):
        """
        Map variables between numeric and categorical representations if needed.

        Args:
            samples (pd.DataFrame): The samples to map.
            reverse (bool): If True, map from numeric to categorical. If False, map from categorical to numeric.
                Defaults to False.

        Returns:
            pd.DataFrame: The mapped samples.
        """
