...
def _check_variables_type(variables):
    for name, var in variables.items():
        ...
        if var.get_dtype() not in allowed_types:
            raise ValueError(
                f"Unrecognized variable type for variable {name}: {var.get_dtype()}." f"Type must be one of {allowed_types}."
            )
...
def map_float_to_variables(data: pd.DataFrame, variables: dict):
    """
    ...
    :type data: pandas.DataFrame
    :param variables_types:
        A dictionary associating the name of each variable to its ValueContainer. T
    :type variables_values: dict
